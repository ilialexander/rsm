function mainAU  (inFile, outFile, displayFlag, learnFlag, learntDataFile)
...
while iteration < (data.N + 1)
    ...
    % visualize the predicted vectors 
     ...
     if (iteration > 1) && (iteration < data.N)
        AU.tolerance = 0;
        index = ismember(AU.inputHistory(:,1:(2*size(SM.input,2))),[SM.inputPrevious SM.input],'rows');
        AU.index = find(index,1);
        if any(AU.index)
            Counts(AU.index) = Counts(AU.index) + 1;
        else
            AU.inputHistory = [AU.inputHistory;
                               SM.inputPrevious SM.input];
            Counts = [Counts;
                        1];
        end
        if (iteration < 100) || (mod(iteration,100) == 0)
            AU.ocurrences = sortrows([Counts AU.inputHistory],'descend');
            AU.inputHistory = AU.ocurrences(:,2:((2*size(SM.input,2))+1));
            Counts = AU.ocurrences(:,1);
        end
        AU.index = ismember(AU.inputHistory(:,1:(size(SM.input,2))),SM.input,'rows');
        AU.predictedInput = AU.inputHistory(find(AU.index,1),:);
