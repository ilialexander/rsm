...
reinforceDendrites = (SM.cellLearn(cellID) == 1); % logical array aligned wth dendrites -- true if corresponding dendrite is connected to active cells
...
%strengthenSynapses = synapse(reinforceSynapses & (SM.synapsePermanence(synapse) < 1)); % Omits saturated permanences
...
SM.synapsePermanence(strengthenSynapses) = SM.synapsePermanence(strengthenSynapses) + SM.P_incr; % Increases permanences
...
if RM.access % Allows to update the relevant variables when RM is accessed
	...
	s = synapse(x);
	s = s (SM.synapsePermanence(s) > 0); % Omits saturated permanences

	SM.synapsePermanence(s) = SM.synapsePermanence(s) - SM.P_decr_pred; % Decreases permanences
