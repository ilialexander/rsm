function mainAUold  (inFile, outFile, displayFlag, learnFlag, learntDataFile)
...
AU.inputHistory = zeros(data.N-1,(2*SM.N));
...
while iteration < 20%(data.N + 1)
    ...
    % visualize the predicted vectors 
     ...
     %% AU
     if (iteration > 1) && (iteration < data.N)
        AU.tolerance = 0;
        AU.inputHistory(iteration,1:(size(SM.input,2))) = SM.inputPrevious;
        AU.inputHistory(iteration,(size(SM.input,2)+1):size(AU.inputHistory,2)) = SM.input;
        [Au, ~, ic] = unique(AU.inputHistory, 'rows', 'stable');
        Counts = accumarray(ic, 1);
        AU.occurencesSorted = [Counts Au];
        AU.occurencesSortedl = flipud(AU.occurencesSorted);
        vaina = AU.occurencesSortedl;
        AU.ocurrences = sortrows([Counts Au],'descend');
        otra = AU.ocurrences;
        if AU.occurencesSortedl == AU.ocurrences
            fprintf('son los mismo');
        end
        AU.index = ismember(AU.ocurrences(:,2:(size(SM.input,2)+1)),SM.input,'rows');
        AU.predictedInput = AU.ocurrences(find(AU.index,1),:);
        if ~isempty(AU.predictedInput)
            ...
            AU.anomalyScore = 1 - nnz(AU.predictedInput(size(SM.input,2)+2:size(AU.predictedInput,2)) & SM.input)/nnz(SM.input);
            
            if AU.anomalyScore == AU.tolerance
                anomalyScores (iteration+1) = 1 - nnz(AU.predictedInput(size(SM.input,2)+2:size(AU.predictedInput,2)) & SM.input)/nnz(SM.input);
